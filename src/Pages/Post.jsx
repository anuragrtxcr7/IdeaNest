      ...
      <div className={`w-full  ${post.status != "Post Anonymous" && "pb-10"}`}>
        <Container>
          {more.length > 0 && post.status != "Post Anonymous" && (
            <h2 className="text-2xl font-semibold flex flex-wrap justify-center">
                ...
                .map(
                  (rec, index) =>
                    index < 3 && (
                      <div
                        key={rec.$id}
                        className="p-4 sm:p-2 w-full border-2 border-slate-900 duration-200 cursor-pointer hover:border-gray-300 rounded-md min-w-64 sm:w-72 md:w-1/4 lg:w-1/5"
                      >
                        <PostCard {...rec} />
                      </div>
                    )
